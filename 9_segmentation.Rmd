...
face_mask = np.zeros(mask_shape, dtype=bool)
in_face_rows, in_face_cols = ski.draw.polygon(snake[:, 0],
                                              snake[:, 1],
                                              mask_shape)
face_mask[in_face_rows, in_face_cols] = True
```
...
We can use a similar trick to make a mask for a circle (or other shapes - see `ski.draw`):

```{python}
circle_rows, circle_cols = ski.draw.disk([100, 220], radius=100, shape=mask_shape)
circle_mask = np.zeros(mask_shape, dtype=bool)
fig, ax = image_show(astronaut)
circle_mask[circle_rows, circle_cols] = True
ax.imshow(circle_mask, alpha=0.3);
```
...
face_flood_mask = seg.flood(astronaut_gray, seed_point, tolerance=0.3)  # Experiment with tolerance

# Display
fig, ax = image_show(astronaut_gray)
ax.imshow(face_flood_mask, alpha=0.3)
ax.plot(seed_point[1], seed_point[0], 'bo');
```

For convenience, we make a function to generate the mask, given the seed and thresholds, and display them.

```{python}
def show_img_mask(img, mask, alpha=0.3):
...
def make_show_flood(img, point, tol):
    """ Calculate and return flood fill mask from img, point and tolerance.
    """
    flood_mask = seg.flood(img, point, tolerance=tol)
    ax = show_img_mask(img, flood_mask)
    ax.plot(point[1], point[0], 'bo')
    return flood_mask
```

Now create and display the food fill for the 0.3 tolerance above:

```{python}
mask = make_show_flood(astronaut_gray, seed_point, 0.3);
```
...
Experiment with the tolerance to find the best value to pick out the
astronaut's face.  We don't think there is a tolerance that does a good job, but do the best you can.

```{python}
# Experiment with the tolerance value.
face_mask = make_show_flood(astronaut_gray, seed_point, 0.3);
```

::: {exercise-end}
:::

::: {solution-start} face-fill-ex
:class: dropdown
:::

We think this is about as good as you can make it:

```{python}
face_mask = make_show_flood(astronaut_gray, seed_point, 0.165)
```
...
This is a rather low-resolution grayscale image of blood vessels in back of
the eye (on the retina).

```{python}
vessels = ski.data.microaneurysms()
plt.imshow(vessels, cmap='gray');
```

Your task is to pick out the vessel running horizontally across the image in the center, using flood fill.

Use the display above to estimate a good seed point, then experiment with seed points and thresholds to pick out the whole vessel.  Here's one very bad experiment:

```{python}
# Use our function to apply flood fill with given seed point and threshold.
vessel_mask = make_show_flood(vessels, (40, 40), 5);
```

::: {exercise-end}
:::

::: {solution-start} eye-artery-ex
:class: dropdown
:::

We found these parameters worked well.  How does our result compare to yours?

```{python}
vessel_mask = make_show_flood(vessels, (50, 42), 15)
```
...
**Warning** : this is a stretch exercise — you will likely have to experiment,
and review this page, and previous pages, to find good solutions.

**Hint 1** : You might want to constrain the search for the right eye to
a particular part of the image, and then refine within this area.

**Hint 2** : Remember, you want a *mask* defining the right eye (not the
boundary).

**Hint 3** : the nose is a strangely difficult problem.  You might consider
trying an edge filtered version of the image to first detect the edges
defining the nose (and other edges), and then find the pixels inside the nose
area from the filtered image.  But any successful method is fine.
...
reye_snake = seg.active_contour(cat_gray, reye_points, alpha=0.6)
