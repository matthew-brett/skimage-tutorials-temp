...
* What if instead of segmenting the head, we segmented the background around
  it, and the collar, and each side of the helmet rim?
* Is there any way to increase the contrast between the background and skin?

```{python}
# These are seeds we used - but you may want to change these.
seed_bkgnd = (100, 350)  # Background
seed_collar = (200, 220)  # Collar
seed_left_rim = (150, 165)  # Left side of helmet rim.
seed_right_rim = (210, 290)  # Right side of helmet rim.
```

```{python tags=c("raises-exception", "remove-output")}
# Make a grayscale image with better contrast.
better_contrast = ski.exposure.equalize_hist(astronaut_gray)
fig, ax = image_show(better_contrast)
```

```{python tags=c("raises-exception", "remove-output")}
tol_bkgnd = 0.14  # Experiment with tolerance for background
flood_background = make_show_flood(better_contrast, seed_bkgnd, tol_bkgnd);
```

```{python tags=c("raises-exception", "remove-output")}
tol_collar = 0.2  # Experiment with tolerance for collar
flood_collar = make_show_flood(better_contrast, seed_collar, tol_collar);
```

```{python tags=c("raises-exception", "remove-output")}
tol_left_rim = 0.115  # Experiment with tolerance for left rim
flood_left_rim = make_show_flood(better_contrast, seed_left_rim, tol_left_rim);
```

```{python}
tol_right_rim = 0.081  # Experiment with tolerance for right rim
flood_right_rim = make_show_flood(better_contrast, seed_right_rim, tol_right_rim);
```

```{python tags=c("raises-exception", "remove-output")}
# Combine the floods with the binary | (OR) operators.
bad_mask = flood_background | flood_collar | flood_left_rim | flood_right_rim
show_img_mask(astronaut_gray, bad_mask);
```

```{python}
# Finally combine the bad areas with the circle mask, using binary AND (&) to get the face.
face_mask = circle_mask & ~bad_mask
show_img_mask(astronaut_gray, face_mask)
```

```{python}
# We could even improve this further by erosion.
better_face = ski.morphology.erosion(face_mask, footprint=ski.morphology.diamond(radius=1))
show_img_mask(astronaut_gray, better_face);
```
